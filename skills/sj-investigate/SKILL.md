...
## Step 4b: 이해 도구 옵션 (마이크로월드)

"에이전트가 대신 고침"과 "사람이 탐색할 도구를 만듦"은 다른 결과를 만든다 —
전자는 빠르지만 사람의 이해가 늘지 않는다. 다음 중 하나면 AskUserQuestion **1회**로
일회용 이해 도구 제작을 제안한다:

- 문제가 **시간에 따른 상태 변화**를 추적해야 이해된다 (실행 순서·경합·누적 상태)
- 가설이 3개까지 갔거나 검증이 2회 이상 실패했다
- 사용자가 "왜 이런지 보고 싶다", "이해가 안 된다"고 표현했다

제안 형태 (문제에 맞는 것 하나만):
- **단계별 실행 스크립트** — 문제 경로를 한 단계씩 실행하며 중간 상태를 출력
- **상태 시각화 HTML** — 데이터 흐름·상태 변화를 브라우저에서 직접 조작하며 관찰
- **before/after 비교 뷰** — 변경 전후 동작을 나란히 재현

규칙: 도구는 스크래치 디렉토리에 만드는 **일회용**이다 — 프로덕션 코드에 넣지 않고,
커밋하지 않는다 ([최소 코드](../_conventions/minimal-code.md) — 이해가 목적, 제품이 아니다).
사용자가 거절하면 즉시 Step 5로 진행 — 재제안 금지.
...
> **컨벤션:** [보고서 옵시디언 정리](../_conventions/obsidian-output.md) — 볼트가 있으면 조사 결과(증상·루트코즈·증거·수정·재발 방지)를 `{볼트}/40_프로젝트/{프로젝트}/보고서/{YYYY-MM-DD} 조사.md`로 저장한다 (PII 마스킹, 같은 날 중복 시 ` -2`). 볼트 없으면 결과 보고에 `미수행: 옵시디언 볼트 없음` 한 줄 — 비차단.
