...
const SHORTCUTS = [
  { id: 'scroll-empty', keys: '↑ / ↓',       action: '채팅 스크롤',              condition: '입력창 비어있을 때' },
  { id: 'history',      keys: '↑ / ↓',       action: '명령 히스토리',             condition: '입력창에 텍스트 있을 때' },
  { id: 'scroll-page',  keys: 'PgUp / PgDn', action: '채팅 5줄 단위 스크롤',      condition: '항상' },
  { id: 'tab-focus',    keys: 'Tab',          action: '포커스 전환 (채팅↔로그)',   condition: '항상' },
  { id: 'ctrl-k',       keys: 'Ctrl+K',       action: '입력 초기화',               condition: '항상' },
  { id: 'ctrl-c',       keys: 'Ctrl+C',       action: '실행 중 태스크 취소',       condition: 'busy일 때' },
  { id: 'help',         keys: '?',            action: '이 도움말',                 condition: '항상' },
  { id: 'quit',         keys: 'q / :q',       action: '종료',                     condition: '입력창 비어있을 때' },
];
...
export function HelpOverlay({ showHelp, onClose }) {
  ...
  return (
    ...
    >
      ...
      {SHORTCUTS.map(({ id, keys, action, condition }) => (
        <Box key={id}>
