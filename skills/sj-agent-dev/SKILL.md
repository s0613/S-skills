...
description: |
  ...
  도구 계층화, 컨텍스트 관리, 가드레일, 옵저버빌리티의 7가지 축을 기준으로
...
| 요청 유형 | 진행 방식 |
|----------|----------|
| 아키텍처 설계 | 7가지 축 체크 → 구조도 제안 → 코드 스캐폴딩 |
...
6. **Observability 설계** — run_id, 로그 항목, 메트릭 정의
7. **구현** — 언어/프레임워크에 맞는 코드 생성
...
프레임워크는 편의도구일 뿐, 7가지 축의 구조 원칙은 동일하게 적용된다.
