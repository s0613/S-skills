...
| 3 | PC 자동화 | 자동화, 자동으로, 매일, 매주, 스케줄, 단축키, launchd, cron, 알림, 파일 이동, 폴더 정리, 앱 실행, 반복, 할 때마다, 되면 자동, shell, 스크립트 | UI 조작 키워드(#2)와 동시 감지 시 #2 우선. **루프 키워드(루프, 계속 돌려줘, 야간에 알아서, 무인으로, 반복 자동화, 스케줄로 돌려) 동시 감지 시 → #22.** 이 행은 *결정론적 스크립트 한 개*를 만드는 요청, #22는 *에이전트가 판단하며 반복하는 루프* 설계 | `Skill("s-skills:sj-automation")` |
