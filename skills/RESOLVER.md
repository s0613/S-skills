...
| 2 | UI 자동화 | 클릭, 버튼, 화면, 스크린, 입력해, UI, 자동 클릭, 이미지 인식, 화면 조작, 화면을, 창을, pyautogui, selenium, 웹 자동화, 브라우저 자동화, 로그인 자동화 | `playwright 테스트`, `playwright 설정`, `e2e 테스트`, `테스트 실행` 포함 시 건너뜀 (테스트 요청). **생성 동사가 그 UI 명사에 직접 걸리면 건너뜀 → #16/#26** (`화면/UI/버튼/컴포넌트 + 만들어줘·디자인해줘·구현해줘·스타일 잡아줘`) — 이 행은 *이미 있는* 화면을 조작하는 요청이고, 화면·UI·버튼을 *만드는* 요청은 디자인이다. 맨 단어(화면·UI·버튼·클릭)가 #16/#26의 구를 부분 매치해 순서로 가로채는 것을 막는다. 단 조작 대상이 이미 있고 만드는 것이 스크립트·매크로면(`화면 클릭하는 매크로 만들어줘`) 이 행을 유지한다. **녹화·촬영 동사(녹화, 찍어, 영상, 동영상, 스크린캐스트, GIF로) 동시 감지 시 건너뜀 → #27** — 화면을 *조작*하는 요청과 화면을 *기록*하는 요청은 다르다. **레퍼런스 조회 동사(레퍼런스, 사례, 다른 앱은, 어떻게 하고 있어, 참고할 만한, 벤치마킹, 유아이볼) 동시 감지 시 건너뜀 → #29** — 화면을 *조작*하는 요청과 남의 화면을 *찾아보는* 요청은 다르다. **안드로이드 기기 키워드(안드로이드, android, 실기기, 폰에서, 휴대폰, 스마트폰, 에뮬레이터, emulator, adb, logcat, APK, 앱 설치) 동시 감지 시 건너뜀 → #30** — 이 행은 *PC 화면*을 조작하는 요청이고, *폰 화면*을 조작하는 요청은 다른 도구가 필요하다 | `Skill("s-skills:sj-automation")` (UI 자동화는 sj-automation에 통합, `/sj-ui-auto`는 그 트리거 별칭) |
| 3 | PC 자동화 | 자동화, 자동으로, 매일, 매주, 스케줄, 단축키, launchd, cron, 알림, 파일 이동, 폴더 정리, 앱 실행, 반복, 할 때마다, 되면 자동, shell, 스크립트 | UI 조작 키워드(#2)와 동시 감지 시 #2 우선. **루프 키워드(루프, 계속 돌려줘, 야간에 알아서, 무인으로, 반복 자동화, 스케줄로 돌려) 동시 감지 시 → #22.** 이 행은 *결정론적 스크립트 한 개*를 만드는 요청, #22는 *에이전트가 판단하며 반복하는 루프* 설계. **녹화·영상 키워드 동시 감지 시 → #27.** **안드로이드 기기 키워드 동시 감지 시 → #30** (폰에서 도는 반복 작업은 PC 스크립트가 아니다) | `Skill("s-skills:sj-automation")` |
...
| 14 | Benchmark | 성능 측정, 벤치마크, benchmark, Core Web Vitals, lighthouse, 로드 타임, 느린 이유 | **안드로이드 기기 키워드 동시 감지 시 → #30** (기기 위 성능·안정성 측정은 실기기 도구가 한다). 웹 성능은 이 행 | `Skill("s-skills:sj-qa")` (benchmark 모드) |
...
| 16 | Design | 디자인 만들어줘, 화면 만들어줘, UI 만들어줘, 페이지 디자인, 컴포넌트 만들어줘, 스타일 잡아줘, 레이아웃 만들어줘, 디자인해줘, 화면 구성, 디자인 구현, UI 구현 | "다양하게", "여러 스타일", "목업 여러 개" 포함 시 → #17. **SEED/당근 디자인 시스템 키워드(#26) 또는 대상 워크스페이스에 `@seed-design/*` 직접 의존성이 있으면 → #26** (디자인 시스템 프로젝트에 자유 디자인은 결함). **만들기 동사 없이 레퍼런스·사례 조회만 요청하면 → #29** (레퍼런스 조회는 만들기의 앞 단계 — 만들어달라는 요청이면 이 행 유지) | `Skill("s-skills:sj-design")` |
...
| 20 | PW Loop | playwright 테스트, playwright 실행, e2e 테스트 실행, e2e 돌려줘, 테스트 통과율, pw-loop, pw 실행, 테스트 돌려줘 | 전제: `playwright.config.ts`/`.js` 존재. 없으면 "playwright 설정 파일이 없습니다" 출력 후 Tiny 경로. **안드로이드 기기 키워드 동시 감지 시 → #30** — Playwright는 브라우저를 몰고, 네이티브 앱 화면은 몰지 못한다 | `Skill("s-skills:pw-loop")` |
...
| 28 | 문서 변환 | 문서 변환, 마크다운으로 변환, markdown으로 바꿔, markitdown, docx, pptx, xlsx, 워드 파일, 엑셀 파일, 파워포인트, PPT 읽어줘, 제안서 읽어줘, 첨부 파일 읽어줘, PDF 전문, epub, 아웃룩 메일, 녹음 전사, 음성 전사, 전사해줘, 유튜브 자막 | 전제 없음 — 미설치면 스킬이 `uvx` 무설치 실행 경로부터 안내. **Read 툴이 이미 읽는 것만 대상이면 비대상** (`.md`·`.txt`·`.json`·`.csv`·짧은 PDF·이미지) — 스킬 Step 0이 되돌려 보낸다. 일반 웹페이지 수집은 WebFetch(비대상), YouTube만 예외. 화면을 *영상으로 남기는* 요청은 #27 | `Skill("s-skills:sj-convert")` |
| 29 | 앱 레퍼런스 | 레퍼런스, 레퍼런스 찾아줘, 사례 보여줘, 다른 앱은 어떻게, 어떤 앱이 쓰고 있어, 참고할 만한 화면, 벤치마킹, 유아이볼, uibowl, UI 사례, 화면 사례, 앱 화면 찾아줘, MAU, 인기 앱, 인기 프로덕트 | 전제: 유아이볼 MCP 등록(미등록 시 스킬이 연결 안내 후 정지). **만들기·구현 동사가 붙으면 조회 후 #16/#26으로 인계**(조회 자체가 목적일 때만 여기서 끝난다). 특정 URL의 비주얼 DNA 추출은 비대상 → #16 | `Skill("s-skills:sj-ref")` |
| 30 | 모바일 실기기 | 안드로이드, android, 실기기, 실 기기, 폰에서, 휴대폰, 스마트폰, 에뮬레이터, emulator, adb, logcat, APK, apk 설치, 앱 테스트, 모바일 테스트, 기기 테스트, 디바이스 테스트, uiautomator, artemis, 아르테미스, 모바일 자동화 | 전제: ARTEMIS 설치 + MCP 등록, USB 디버깅 켠 기기 또는 에뮬레이터(미충족 시 스킬이 설치·연결 안내 후 정지). **iOS·아이폰·시뮬레이터는 비대상** — 스킬이 미지원을 알리고 멈춘다. **만들기 동사가 모바일 화면에 걸리면 → #16/#26** (`앱 화면 만들어줘`는 디자인). **남의 앱 화면 사례 조회는 → #29**. 합격/불합격 **판정**은 이 행이 아니라 #13/#14 또는 sj-qa — 이 스킬은 증거 수집기다 | `Skill("s-skills:sj-mobile")` |
