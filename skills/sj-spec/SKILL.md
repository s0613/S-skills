...
위 6개는 **평문 메시지로 한 번에 보여준다** — AskUserQuestion은 호출당 최대 4문항이라 6개를 한 번에 담을 수 없다.

사용자가 자유 서술로 답하면 그대로 쓴다. 답이 비어 있거나 일부만 오면 **다시 묻지 않고** 나머지는 가정으로 채운다. 가정 중 스펙 방향을 크게 가르는 것이 있으면 그때만 AskUserQuestion 1회로 **최대 4문항**을 골라 확인한다 (우선순위: 성공 지표 → 스코프 축소 여부 → 주 사용자 → 기술 제약).

모든 가정은 스펙 문서 `## 가정` 절에 `- {항목}: {가정} (사용자 미답변)`으로 남긴다 — 은폐하지 않는다([정직 산출 계약](../_conventions/honest-report.md)).
