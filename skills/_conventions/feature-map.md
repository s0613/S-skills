...
기계가 확인할 수 있는 건 **자기모순**까지다(참조 무결성, 표↔mermaid 일치). "F02가 정말
F01에 의존하는가"는 확인하지 못한다. 완전 자동화는 불가능하지만 **표본은 가능하다.**

sj-qa가 판정할 때 지도의 `의존` 엣지 **1개를 무작위로 골라** 실제 코드와 대조한다:

```bash
# 엣지 하나 뽑기
awk -F'|' '/^\| *F[0-9]/ {gsub(/ /,"",$2); n=split($7,a,/[ ,]+/); for(i=1;i<=n;i++) if(a[i] ~ /^F[0-9]+$/) print $2, a[i]}' \
  docs/FEATURE-MAP.md | sort -R | head -1
```

뽑힌 `A → B`에 대해 A의 `핵심 파일`이 B의 `핵심 파일`을 실제로 import·호출·공유하는지 본다.

- **확인됨** → 기록만 하고 넘어간다.
- **반증됨**(그런 관계가 코드에 없다) → **LOW 이슈**로 기록하고 해당 행을 지목한다. FAIL 사유가 아니다.
- **판단 불가**(의미적 의존이라 코드로 안 보임) → `미확인`으로 기록한다. 이것도 정보다.

한 번에 1개라 비용이 거의 없고, 실행이 쌓이면 커버리지가 자연히 올라간다.
**전수 검증을 기다리다 아무것도 안 하는 것보다 낫다.**
