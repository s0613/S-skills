...
**작성 규칙**
- 표를 채우기 전에 규칙 정본의 "칸 규칙" 절을 읽는다 — `핵심 파일`은 최대 3개이고 여러 개는 **공백 또는 쉼표로 구분**하며 디렉토리는 끝에 `/`를 붙인다(drift 검사가 이 구분자로 쪼갠다). `테스트`가 없으면 빈칸이 아니라 `없음`.
- 라우트·엔트리포인트에서 출발한다. 단순 유틸리티 함수는 기능이 아니다.
- **2차 훑기 — 라우트 없는 공유 인프라 (건너뛰지 않는다)**: 라우트에서만 출발하면 여러 기능이
  공유하는 라우트 없는 코드(알림 팬아웃, 공용 클라이언트, 스케줄러 등)를 통째로 놓친다.
  실제로 이 규칙만으로 만든 지도가 `src/lib/notify/`를 놓쳐, 그 파일을 고친 버그에서 수정
  지점으로 안내하지 못했다. 1차 표를 만든 뒤 반드시 한 번 더 훑는다:

  ```bash
  # 표의 핵심 파일들이 import하는 내부 모듈을 빈도순으로 (경로는 프로젝트에 맞게)
  grep -rhoE "from ['\"](@/|\.\./|\./)[^'\"]+" src \
    | sed "s/.*from ['\"]//" | sort | uniq -c | sort -rn | head -25
  ```

  빈도만으로 올리면 안 된다 — 실제 프로젝트에서 상위를 차지하는 건 `ui/button`,
  `types/result`, `lib/utils`, `supabase/client` 같은 **범용 기반**이고, 이걸 기능 행으로
  올리면 지도가 더 나빠진다. 두 조건을 **모두** 만족할 때만 올린다:

  1. 2개 이상의 기능 행이 import하고,
  2. **도메인 규칙을 담고 있다** — 누가·언제·무엇을 받는지를 그 모듈이 결정한다.
     제외: UI 프리미티브, 타입·상수, 포맷터, DB·HTTP 클라이언트 래퍼, 로거.

  판별 질문: **"이 모듈만 고치는 버그 수정이 있었을 법한가?"**
  알림 팬아웃·권한 판정·가격 계산·상태 전이는 그렇다. 버튼·날짜 포맷터는 아니다.

  올릴 때: `진입점`은 그 모듈의 대표 파일, `핵심 파일`은 그 디렉토리, `의존`은 보통 비운다
  (의존하는 쪽이 아니라 의존당하는 쪽이므로). 대신 **그것을 쓰는 기능들의 `의존` 칸에 이 새
  ID를 추가**한다 — 그래야 그 기능이 깨졌을 때 지도가 여기로 안내한다.

  두 조건 중 하나만 맞으면 올리지 말고, 애매하면 `## 미매핑`에 **이유와 함께** 남긴다.
- `## 미매핑`에 **"페이지가 아니라서 제외"라고 쓰지 않는다.** 미매핑은 "기능인지 확신이 없는 것"을
  담는 곳이지 "라우트가 없어서 뺀 것"을 담는 곳이 아니다 — 후자를 여기 넣으면 놓친 기능이
  검토된 것처럼 보인다. 라우트 없는 코드는 2차 훑기로 판정하고, 그래도 애매하면 이유를 적어 남긴다.
