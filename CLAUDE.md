...
- **s-skills:sj-company** (`/sj-company`) — 상태/의도 기반 라우터 v3.8.1. 라우팅 키워드의 단일 사실은 [skills/RESOLVER.md](skills/RESOLVER.md) — Step 0이 런타임에 읽어 디스패치 (행 수·키워드는 RESOLVER 표가 단일 사실). 모호성 해소는 행위(동사) 우선 — 키워드가 대상만 가리키면 행위 행으로. RUN_ID 파이프라인 추적. ship 호출 전 브랜치 확인 필수.
...
- **s-skills:sj-tech-lead** (`/tech-lead`) — 전문 개발 서브에이전트를 병렬 디스패치하고 통합·리뷰 v2.5.1. RUN_ID 연결. learned 패턴 자동 로딩. [SPEC:] 참조 자동 인식. PII 마스킹 적용. 리뷰어 다양성 컨벤션(렌즈 분리 + 심각도 보정) 적용. 7a-1 CRITICAL 적대 검증에 GPT 교차모델 렌즈(codex MCP, best-effort) 추가 — 단일 AI 리뷰어 비차단.
